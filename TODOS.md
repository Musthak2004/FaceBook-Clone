...
## Design system & 3D UI (P1) — COMPLETE
...
- [x] T1: Extract shared post-card include template from home.html and post_list.html (DRY)
- [x] T2: CSS foundation — .no-js gating, transition defaults, will-change, @supports guards
- [x] T3: Add VanillaTilt.js v1.8.0 vendor library to static/vendor/
- [x] T4: Create depth.css — card tilt, glass navbar, avatar translateZ, parallax, dark mode
- [x] T5: Create feed-3d.js — VanillaTilt init, MutationObserver, IntersectionObserver, touch/Safari detection
- [x] T6: Blue-tinted glass navbar — backdrop-filter: blur(12px), rgba(24,119,242,0.75)
- [x] T7: Safari CSS-only fallback (no VanillaTilt JS on Safari)
- [x] T8: AJAX load-more JSON endpoint (GET /api/posts/?offset=N) + full states (loading, error, exhausted)
- [x] T9: Avatar placeholder initials with translateZ(20px)
- [x] T10: Keyboard tilt (:focus-within + :focus-visible outline)
- [x] T11: Touch device CSS float animation + 44px tap targets
- [x] T12: Typography integration — Satoshi via Fontshare, Instrument Serif + JetBrains Mono via Google Fonts
- [x] T13: Login page depth — glass form card, logo translateZ
- [x] T14: Overlay z-stack — modals/dropdowns outside perspective container
- [x] T15: Performance test — 60fps scroll, backdrop-filter <5% frame drops, FPS meter check
